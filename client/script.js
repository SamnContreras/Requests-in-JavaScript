...
sayHelloButton.addEventListener("mouseover", blackBackground)
...
function changeBBackground (event){
    event.target.classList.toggle("changeBBackground")
}
sayHelloButton.addEventListener("mouseout", changeBBackground)
...
function request () {
    axios.get("http://localhost:3000/query-test?sam")
    .then(function(response){
        console.log(response.data)
    })
}
